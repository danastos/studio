...
contract TheStudioToken is ERC20, Projects  {
    ...
    event newMediaToken ( string _name , address _address , string _symbol );
    ...
    function TheStudioToken() {
        ...
        uint256 _initialSupply = 500000000000; // THIS HERE MAY NEED TO BE ADJUSTED..
     ...
     function studioLevel ( address _address ) public constant returns(string){
        ...
        if ( balanceOf [ _address] < associateproducer * 10000 ) return "FAN";
        if ( balanceOf [ _address] < producer * 10000  ) return "ASSOCIATE PRODUCER";
        if ( balanceOf [ _address] < executiveproducer * 10000  ) return "PRODUCER";
    ...
    function newMediaToken ( uint256 _initialSupply, uint8 _decimalUnits, string _name, string _symbol ) public onlyManagement {
        ...
        mediaTokenCount++;
        newMediaToken ( _name , _mediaToken , _symbol );
        
        //mediaTokenCrowdSale = new Crowdsale( _mediaToken, msg.sender );
